    ...
    return (
        <div className='bg-[#1A1C1B] flex items-center justify-between px-6'>
            ...
            <div className='flex gap-6'>
                {state?.token && <button onClick={() => router.push('/add-section')}>Add Section</button>}
                {state?.user?.role === 'author' && <button onClick={() => router.push('/manage-access')}>Manage Access</button>}
