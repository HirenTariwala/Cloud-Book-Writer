...
export default function useManageAccessHook() {
    ...
    const onChangeHandler = (e: React.FormEvent<HTMLSelectElement>) => {
        ...
        const collaborator: SelectedCollaboratorType[] = users.filter((user: SelectedCollaboratorType) => user?.id === +e.currentTarget?.value)
    ...
    const updatePermissions = async() => {
        if (selectedCollaborator?.id) {
            const response = await updateUserPermission({ id: selectedCollaborator?.id, permissions})
